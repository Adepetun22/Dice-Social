...
const EventInfoModal = ({
  ...
  const handleJoinEventClick = () => {
    // Show EventFormModal when Join Event button is clicked
    setShowEventFormModal(true);
    // Optionally call the onJoinEvent callback
    if (onJoinEvent) {
      onJoinEvent();
    }
  };
  ...
  const handleEventFormSubmit = (formData, eventName) => {
    console.log('Event registration submitted:', formData, eventName);
    // Close both modals
    setShowEventFormModal(false);
    if (onClose) {
      onClose();
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            <button
              className="rounded-lg border-solid border-[#FFD700] border pt-3 pr-2 pb-3 pl-2 flex flex-row gap-2.5 items-center justify-center flex-1 h-9 min-w-[99px] max-w-[128px] relative cursor-pointer transition-colors"
            ...
            <button
              className="rounded-lg pt-3 pr-2 pb-3 pl-2 flex flex-row gap-2.5 items-center justify-center shrink-0 w-32 h-9 min-w-[99px] max-w-[128px] relative cursor-pointer transition-opacity"
