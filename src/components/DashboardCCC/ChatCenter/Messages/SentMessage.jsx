...
const SentMessage = forwardRef(({ 
  message, 
  showDropdown, 
  onDropdownToggle,
  dropdownPosition = { left: '-100px', top: '20px' },
  messageStatus = true,
  isNew,
  file
}, ref) => {
  ...
  const renderMediaContent = () => {
    if (!file) {
      return (
        <div className="message2 text-sm break-words whitespace-pre-wrap">
          {message}
        </div>
      );
    }
      ...
      case 'photo':
        return (
          <div className="flex flex-col">
            {message && (
              <div className="message2 text-sm break-words whitespace-pre-wrap mb-2">
                {message}
              </div>
            )}
        ...
        return (
          <div className="flex flex-col">
            ...
            <div className="max-w-[320px] max-h-[320px] w-full overflow-hidden rounded-lg relative">
              <video className="w-full h-full object-contain cursor-pointer" onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <div className={`send-message mb-4 flex justify-end ${isNew ? 'animate-push-up' : ''}`}>
      <div 
        className="component-284 flex relative"
        ref={ref}
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
      >
        <button 
          className={`dots-three-outline-vertical3 w-4 h-4 mr-2 self-start cursor-pointer transition-opacity duration-200 ${
        ...
        {messageStatus && (
          <svg className="message-status w-4 h-4 ml-2 self-end" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
